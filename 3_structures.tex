\section{Estructuras}
  ...
  %   Teo$_{0}$ es trivial. Veamos que Teo$_{k}$ implica Teo$_{k+1}$. Supongamos que vale Teo$_{k}$ y supongamos $F:\mathbf{A} \rightarrow \mathbf{B}$ es un homomorfismo, $t\in T_{k+1}^{\tau }-T_{k}^{\tau }$ y $\vec{a} =(a_{1},a_{2},...)\in A^{\mathbb{N}}$. Denotemos $(F(a_{1}),F(a_{2}),...)$ con $F(\vec{a})$. Por Lema 117, $t=f(t_{1}, \dotsc, t_{n})$, con $n\geq 1 $,$\;f\in \mathcal{F}_{n}$ y $t_{1}, \dotsc, t_{n}\in T_{k}^{\tau }$. Tenemos entonces
  %
  ...
  %     (F(a_{1}), F(a_{2}), \dotsc)]
  %   \]
  %   \PN para cada $(a_{1}, a_{2}, \dotsc) \in A^{\mathbb{N}}$. En particular $\mathbf{A}$ y $\mathbf{B}$ satisfacen las
