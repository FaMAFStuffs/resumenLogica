\section{Teorias de primer orden}
  ...
  % % Lemma 149. Con prueba. Lemma 61.
  % \begin{lemma}
  %   \PN Si $\varphi$ se deduce de $\psi$ por la regla de generalización, entonces el nombre de constante $c$ del cual
  %   habla la propiedad que define al conjunto $Generaliz^{\tau}$ está univocamente determinado por el par $(\varphi,
  %   \psi)$.
  % \end{lemma}
  % \begin{proof}
  ...
  % \end{proof}
  %
  % % Lemma 150. Con prueba. Lemma 62.
  % \begin{lemma}
  %   \PN Si $\varphi$ se deduce de $\psi$ por la regla de elección, entonces el nombre de constante $e$ del cual habla la
  %   propiedad que define al conjunto $Elec^{\tau}$ está univocamente determinado por el par $(\varphi, \psi)$.
  % \end{lemma}
  % \begin{proof}
  %   Notese que $e$ es el unico nombre de constante que ocurre en $\varphi$ y no ocurre en $\psi$.
  % \end{proof}
  %
  % % Lemma 151. Con prueba. Lemma 63.
  % \begin{lemma}
  ...
  % \end{lemma}
  % \begin{proof}
  %   Veamos que la regla de existencia es universal. Supongamos $\varphi =_{d}\varphi (v)$, $t\in T_{c}^{\tau }$ y $\mathbf{A}$ es una estructura de tipo $\tau $ tal que $\mathbf{A}\models \varphi (t)$. Sea $t^{\mathbf{A}}$ el valor que toma $t$ en $\mathbf{A}$. Por el Lema 148 tenemos que $\mathbf{A}\models \varphi \left\lbrack t^{\mathbf{A}}\right\rbrack $, por lo cual tenemos que $\mathbf{A}\models \exists v\varphi (v)$.
  %
  ...
  % \end{proof}
  %
  % % Lemma 152. Con prueba. Lemma 64.
  % \begin{lemma}
  %   \PN Sea $\mathbf{\varphi} \in S^{\tau +}$. Hay únicos $n \geq 1$ y $\varphi_{1}, \dotsc, \varphi_{n} \in S^{\tau}$
  %   tales que $\mathbf{\varphi} = \varphi_{1} \dotsc \varphi_{n}$.
  % \end{lemma}
  % \begin{proof}
  %   Solo hay que probar la unicidad la cual sigue de la Proposicion 128.
  % \end{proof}
  %
  % % Lemma 153. Con prueba. Lemma 65.
  % \begin{lemma}
  %   \PN Sea $\mathbf{J} \in Just^{+}$. Hay únicos $n \geq 1$ y $J_{1}, \dotsc, J_{n} \in Just$ tales que $\mathbf{J} =
  %   J_{1} \dotsc J_{n}$.
  % \end{lemma}
  % \begin{proof}
  %   Supongamos $J_{1}, \dotsc, J_{n}$, $J_{1}^{\prime}, \dotsc, J_{m}^{\prime}$, con $ n,m\geq 1$, son justificaciones tales que $J_{1}\dotsc J_{n}=J_{1}^{\prime}\dotsc J_{m}^{\prime}$. Es facil ver que entonces tenemos $J_{1}=J_{1}^{\prime}$, por lo cual $J_{2}\dotsc J_{n}=J_{2}^{\prime}\dotsc J_{m}^{\prime}$. Un argumento inductivo nos dice que entonces $n=m$ y $J_{i}=J_{i}^{\prime}$, $ i=1, \dotsc, n$ $\Box$
  % \end{proof}
  %
  % % Lemma 154. Con prueba. Lemma 66.
  % \begin{lemma}
  %   \PN Sea $(\mathbf{\varphi}, \mathbf{J})$ una prueba de $\varphi$ en $(\Sigma, \tau)$.
  %   \begin{enumerate}\lbrack(1)\rbrack
  %     \item Sea $m \in \mathbb{N}$ tal que $\mathbf{J}_{i} \neq \mathrm{ HIPOTESIS}\bar{m}$, para cada $i =
  %     1, \dotsc, n(\mathbf{\varphi})$. Supongamos que $\mathbf{J}_{i} = \mathrm{HIPOTESIS}\bar{k}$ y que $\mathbf{J}_{j}
  ...
  %     justificación $\mathbf{J}_{j}$ por $\mathrm{TESIS}\bar{m}\alpha$, entonces $(\mathbf{\varphi},
  %     \mathbf{\tilde{J}})$ es una prueba de $\varphi$ en $(\Sigma, \tau)$.
  %     \item Sea $\mathcal{C}_{1}$ el conjunto de nombres de constante que ocurren en alguna $\mathbf{\varphi}_{i}$ y que
  ...
  %     a)$ es un tipo. Sea $\tilde{\varphi}_{i} =$ resultado de reemplazar en $\mathbf{\varphi}_{i}$ cada ocurrencia de
  %     $e$ por $\tilde{e}$, entonces $(\mathbf{\tilde{\varphi}}_{1} \dotsc \mathbf{\tilde{\varphi}}_{n(\mathbf{\varphi})},
  ...
  %   (2) Supongamos que $(\Sigma, \tau)\vdash \varphi_{1}, \dotsc, \varphi_{n}$ y que $\varphi$ se deduce por regla R a partir de $\varphi_{1}, \dotsc, \varphi_{n}$, con R universal. Notese que
  %
  ...
  % \end{proof}
  %
  % % Theorem 156. Sin prueba. Theorem 68.
  % \begin{theorem}
  %   \PN \textbf{(Corrección)} $(\Sigma, \tau) \vdash \varphi \Rightarrow (\Sigma, \tau) \models \varphi$.
  % \end{theorem}
  %
  % % Corollary 157. Con prueba. Corollary 69.
  % \begin{corollary}
  ...
  % \end{proof}
  %
  % % Lemma 159. Con prueba. Lemma 71.
  % \begin{lemma}
  %   \PN Dada una teoria $(\Sigma, \tau)$, el par $(S^{\tau}/\mathrm{\dashv \vdash}, \leq)$ es un reticulado en el cual:
  %   \begin{eqnarray*}
  %     \lbrack \varphi \rbrack \ \SU \ \lbrack\psi \rbrack &=& \lbrack(\varphi \vee \psi)\rbrack \\
  %     \lbrack \varphi \rbrack \ \IN \ \lbrack\psi \rbrack &=& \lbrack(\varphi \wedge \psi)\rbrack
  %   \end{eqnarray*}
  %   \PN Más aún $(S^{\tau}/\mathrm{\dashv \vdash}, \SU, \IN$ es distributivo.
  % \end{lemma}
  % \begin{proof}
  %   Primero que todo deberemos verificar que $\leq $ es un orden parcial sobre $ S^{\tau }/\dashv \vdash $. Veamos que $\leq $ es antisimetrica las otras dos propiedades son dejadas al lector. Supongamos que $\lbrack\varphi \rbrack\leq \lbrack \psi \rbrack$ y $\lbrack\psi \rbrack\leq \lbrack \varphi \rbrack.$ Es decir que $(\Sigma, \tau)\vdash \left( \varphi \rightarrow \psi \right) $, $\left( \psi \rightarrow \varphi \right) .$ Notese que
  %
  ...
  %
  %   justifica que $(\{\left( \varphi \rightarrow \psi \right) ,\left( \psi \rightarrow \varphi \right) \},\tau )\vdash (\varphi \leftrightarrow \psi )$ , lo cual por el Lema 155 nos dice que $(\Sigma, \tau)\vdash (\varphi \leftrightarrow \psi )$, obteniendo $\lbrack\varphi \rbrack=\lbrack\psi \rbrack.$
  %   Veamos ahora que $\lbrack(\varphi \vee \psi )\rbrack=\lbrack\varphi \rbrack\;\mathsf{s\;}\lbrack\psi \rbrack.$ Es facil probar que $\lbrack\varphi \rbrack,\lbrack\psi \rbrack\leq \lbrack (\varphi \vee \psi )\rbrack.$ Supongamos que $\lbrack\varphi \rbrack,\lbrack\psi \rbrack\leq \lbrack \alpha \rbrack.$ Es decir que $ (\Sigma, \tau)\vdash (\varphi \rightarrow \alpha ),(\psi \rightarrow \alpha ).$ Notese que
  %
  ...
  %
  %   justifica que $(\{(\varphi \rightarrow \alpha ),(\psi \rightarrow \alpha )\},\tau )\vdash (\varphi \vee \psi )\rightarrow \alpha $ lo cual por el Lema 155 nos dice que $(\Sigma, \tau)\vdash (\varphi \vee \psi )\rightarrow \alpha $, obteniendo que $\lbrack(\varphi \vee \psi )\rbrack\leq \lbrack \alpha \rbrack.$
  ...
  %   $\displaystyle \lbrack (\varphi \wedge (\psi \vee \varphi ))\rbrack=\lbrack((\varphi \wedge \psi )\vee (\varphi \wedge \varphi ))\rbrack. $
  %
  %   Se tiene entonces
  %   $\displaystyle \begin{array}{lll} \lbrack \varphi \rbrack\;\mathsf{i}\;(\lbrack\psi \rbrack\;\mathsf{s}\;\lbrack\varphi \rbrack) & = & \lbrack\varphi \rbrack\;\mathsf{i}\;(\lbrack(\psi \vee \varphi )\rbrack) \\ & = & \lbrack(\varphi \wedge (\psi \vee \varphi ))\rbrack \\ & = & \lbrack((\varphi \wedge \psi )\vee (\varphi \wedge \varphi ))\rbrack \\ & = & \lbrack(\varphi \wedge \psi )\rbrack\;\mathsf{s}\;\lbrack(\varphi \wedge \varphi )\rbrack \\ & = & (\lbrack\varphi \rbrack\;\mathsf{i}\;\lbrack\psi \rbrack)\;\mathsf{s}\;(\lbrack\varphi \rbrack\;\mathsf{i} \;\lbrack\varphi \rbrack) \end{array} $
  %
  %   El resto de la prueba es dejado al lector.
  % \end{proof}
  %
  % % Lemma 160. Con prueba. Lemma 72.
  % \begin{lemma}
  %   \PN El poset $(S^{\tau}/\mathrm{\dashv \vdash}, \leq)$ tiene $0$ y $1$ dados por:
  %   \begin{eqnarray*}
  %     0 &=& \{\varphi \in S^{\tau}: \varphi \text{ es refutable en } (\Sigma, \tau)\} \\
  %     &=& \lbrack \varphi \rbrack, \text{ para cada } \varphi \text{ refutable} \\
  %     && \\
  %     1 &=& \{\varphi \in S^{\tau}: \varphi \text{ es refutable en } (\Sigma, \tau)\} \\
  %     &=& \lbrack \varphi \rbrack, \text{ para cada teorema } \varphi
  %   \end{eqnarray*}
  % \end{lemma}
  % \begin{proof}
  %   Veamos que para cada $\varphi$ tal que $(\Sigma, \tau)\vdash \lnot \varphi ,$
  %
  %   $\displaystyle \lbrack \varphi \rbrack=\{\varphi \in S^{\tau }:(\Sigma, \tau)\vdash \lnot \varphi \} $
  %
  ...
  %
  %   justifica que $(\{\lnot \varphi ,\lnot \psi \},\tau )\vdash (\varphi \leftrightarrow \psi )$ lo cual por el Lema 155 nos dice que $(\Sigma, \tau)\vdash (\varphi \leftrightarrow \psi )$, obteniendo que $ \psi \in \lbrack \varphi \rbrack.$
  ...
  %
  %   $\displaystyle \{\varphi \in S^{\tau }:(\Sigma, \tau)\vdash \varphi \}=\lbrack\varphi \rbrack, $
  %
  %   es un $1$ del poset $(S^{\tau }/\mathrm{\dashv \vdash},\leq ).$
  % \end{proof}
  %
  % % Lemma 161. Con prueba. Lemma 73.
  % \begin{lemma}
  %   \PN $(S^{\tau}/\mathrm{\dashv \vdash}, \SU, \IN, ^{c}, 0, 1)$ es un álgebra de Boole.
  % \end{lemma}
  % \begin{proof}
  ...
  % \end{lemma}
  % \begin{proof}
  %   (1) Supongamos $(\Sigma, \tau)\vdash \varphi $. Entonces hay una prueba $ (\varphi_{1}\dotsc\varphi_{n},J_{1}\dotsc J_{n})$ de $\varphi$ en $(\Sigma, \tau)$. Sea $\mathcal{C}_{1}$ el conjunto de nombres de constante que ocurren en alguna $\varphi_{i}$ y que no pertenecen a $\mathcal{C}.$ Notese que aplicando varias veces el Lema 154 podemos obtener una prueba $ (\tilde{\varphi}_{1}\dotsc\tilde{\varphi}_{n},J_{1}\dotsc J_{n})$ de $\varphi$ en $(\Sigma, \tau)$ la cual cumple que los nombres de constante que ocurren en alguna $\psi_{i}$ y que no pertenecen a $\mathcal{C}$ no pertenecen a $ \mathcal{C}^{\prime}$. Pero entonces $(\tilde{\varphi}_{1}\dotsc\tilde{\varphi} _{n},J_{1}\dotsc J_{n})$ es una prueba de $\varphi$ en $(\Sigma ,\tau ^{\prime})$, con lo cual $(\Sigma ,\tau ^{\prime})\vdash \varphi $
  %
  ...
  % \end{proof}
  %
  % % Lemma 163. Con prueba. Lemma 75.
  % \begin{lemma}
  %   \PN Sea $(\Sigma, \tau)$ una teoría y supongamos que $\tau$ tiene una cantidad infinita de nombres de cte que no
  %   ocurren en las sentencias de $\Sigma$, entonces para cada formula $\varphi =_{d} \varphi(v)$, se tiene que $\lbrack
  %   \forall v\varphi (v) \rbrack = \inf(\{\lbrack \varphi(t) \rbrack: t$ es un término cerrado$\})$.
  % \end{lemma}
  % \begin{proof}
  ...
  %     \[
  %       \mathbf{A} \models \varphi \lbrack \vec{a} \rbrack \Leftrightarrow \mathbf{A}^{\prime} \models \varphi \lbrack
  %       \vec{a} \rbrack
  %     \]
  %     \item Si $\Sigma \cup \{\varphi\} \subseteq S^{\tau_{\cap}}$, entonces:
  %     \[
  %       (\Sigma, \tau) \models \varphi \Leftrightarrow (\Sigma, \tau^{\prime}) \models \varphi
  %     \]
  %   \end{enumerate}
  % \end{lemma}
  % \begin{proof}
  ...
  % \end{proof}
  %
  % % Lemma 165. Con prueba. Lemma 77.
  % \begin{lemma}
  %   \PN Sea $\tau$ un tipo. Hay una sucesión de fórmulas:
  %   \[
  %     \gamma_{1}, \gamma_{2}, \dotsc
  %   \]
  %   \PN tal que:
  %   \begin{enumerate}
  ...
  % \end{proof}
  %
  % % Theorem 166. Con prueba. Theorem 78.
  % \begin{theorem}
  %   \PN \textbf{(Completitud) (G{\"o}del)} $(\Sigma, \tau) \models \varphi \Rightarrow (\Sigma, \tau) \vdash \varphi$.
  % \end{theorem}
  ...
  % \end{proof}
  %
  % % Corollary 167. Con prueba. Corollary 79.
  % \begin{corollary}
  ...
  % \end{proof}
  %
  % % Corollary 168. Con prueba. Corollary 80.
  % \begin{corollary}
  ...
  % \end{proof}
  %
  % % Lemma 172. Con prueba. Lemma 83.
  % \begin{lemma}
  %   \PN Las siguientes reglas de la lógica ecuacional son universales.
  %   \begin{itemize}
  %     \item TRANSITIVIDAD
  %     \begin{eqnarray*}
  ...
  %       s \approx t
  %     \end{eqnarray*}
  %     \item SUBTITUCIÓN % TODO
  %     \begin{eqnarray*}
  %       && \underline{t \approx s} \qquad \text{ con } t =_{d} t(x_{1}, \dotsc, x_{n}) \text{ y } s =_{d} s(x_{1},
  %         \dotsc, x_{n}) \\
  %       && t(p_1, \dotsc, p_n) \approx s(p_1, \dotsc, p_n)
  %     \end{eqnarray*}
  %     \item REEMPLAZO
  %     \begin{eqnarray*}
  %       && \underline{t \approx s} \qquad \text{ donde } \tilde r \text{ es el resultado de reemplazar } \\
  %       && r \approx \tilde r \qquad \text{ algunas ocurrencias de } t \text{ en } r \text{ por } s
  %     \end{eqnarray*}
  %   \end{itemize}
  % \end{lemma}
  % \begin{proof}
  ...
  % \end{proof}
  %
  % % Theorem 173. Con prueba. Theorem 84.
  % \begin{theorem}
  ...
  % \end{proof}
  % 
  % % Theorem 174. Con prueba. Theorem 85.
  % \begin{theorem}
  %   \PN \textbf{(Completitud) (Birkhoff)} Sea $(\Sigma, \tau)$ una teoría tal que los elementos de $\Sigma$ son
  %   identidades. Si $(\Sigma, \tau) \models p \approx q$, entonces $(\Sigma, \tau) \vdash_{ec} p \approx q$.
  % \end{theorem}
  % \begin{proof} % TODO error de compilación.
